...
var phpmd = function phpmd(data) {
  return vile.spawn(PHPMD, { args: phpmd_args(data) }).then(function (spawn_data) {
    var stdout = _.get(spawn_data, "stdout", "");
    return xml_to_json(stdout);
  }).then(function (phpmd_result) {
    return _.get(phpmd_result, "pmd.file", []);
  });
};

var issue_type = function issue_type(violation) {
  return _.get(violation, "$.priority") == 1 ? vile.MAIN : vile.STYL;
};
...
var signature = function signature(violation) {
  return _.has(violation, "$.ruleset") || _.has(violation, "$.rule") ? "phpmd::" + _.get(violation, "$.ruleset") + ("::" + _.get(violation, "$.rule")) : "phpmd::" + _.trim(_.get(violation, "_"));
};

var line_info = function line_info(violation) {
  var start = start_line(violation);
  var end = end_line(violation);
